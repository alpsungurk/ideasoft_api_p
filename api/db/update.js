// Vercel Serverless Function - Unified update endpoint
// Supports: batch-stats, status, category updates
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!action) {
      return res.status(400).json({ success: false, error: 'action parametresi gerekli (batch-stats, status, category)' })
    }
    ...
    switch (action) {
      ...
      case 'status': {
        const { sku, ideasoftId, status, error } = data
        if (!sku) {
      ...
      case 'category': {
        const { sku, categoryId, categoryName } = data
        if (!sku) {
          return res.status(400).json({ success: false, error: 'sku gerekli' })
        }
      ...
      default:
        return res.status(400).json({ success: false, error: 'Geçersiz action. Desteklenen: batch-stats, status, category' })
    }
  } catch (error) {
